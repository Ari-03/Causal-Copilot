...
from global_setting.state import GlobalState, Logging
...
def global_state_initialization(args: argparse.Namespace = None) -> GlobalState:
    ...
    # Initialize all logging fields
    global_state.logging = Logging(
        query_conversation=[{"response": user_query}],
        knowledge_conversation=[],
        filter_conversation=[],
        select_conversation=[{"response": user_query}],
        argument_conversation=[{"response": user_query}],
        errors_conversion=[],
        graph_conversion={},
        downstream_discuss=[],
        final_discuss=[],
    ...
    algorithms = [
        algo.split(".")[0]
        for algo in os.listdir("algorithm/context/algos")
        if algo.endswith(".txt") and "tagging" not in algo and "guideline" not in algo
    ]
    ...
    client = OpenAI()
    prompt = (
        f"Based on the query that I provided: {user_query} \n\n; "
    ...
    if args.demo_mode:
        ...
        global_state.user_data.output_report_dir = (
            f"output/{args.data_file.split('/')[-1]}/{date_time}/output_report"
        )
        global_state.user_data.output_graph_dir = (
            f"output/{args.data_file.split('/')[-1]}/{date_time}/output_graph"
        )
